...
def get_common_bird_name(scientific_name):
    ...
    if result:
        ...
        logging.warn("No common bird name for: " + scientific_name)
        return ""


def classify(image, is_bird):
...
def on_disconnect(client, userdata, rc):
    if rc != 0:
        logging.warn("Unexpected disconnection, trying to reconnect")
        while True:
            try:
                client.reconnect()
                break
            except Exception as e:
                logging.warn(f"Reconnection failed due to {e}, retrying in 60 seconds")
                time.sleep(60)
    else:
...
def set_sublabel(frigate_url, frigate_event, sublabel):
    ...
    logging.debug(f'sublabel: {sublabel}')
    logging.debug(f'sublabel url: {post_url}')
    ...
    if response.status_code == 200:
        logging.info("Sublabel set successfully to: " + sublabel)
    else:
        logging.error("Failed to set sublabel. Status code:", response.status_code)
    ...
    if not firstmessage:
        ...
        if (after_data['camera'] in config['frigate']['camera'] and bird_or_dog):
            ...
            if response.status_code == 200:
                ...
                categories = classify(np_arr, is_bird)
                ...
                if index != 964 and score > classification_config['threshold']:  # 964 is "background"
                    ...
                    if result is None:
                        ...
                        logging.info("There is already a record for this event. Checking score")
                        ...
                        if score > existing_score:
                            ...
                            logging.info("New score is lower.")
        ...
        firstmessage = False
        logging.debug("skipping first message")
